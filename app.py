...
if not st.session_state["authenticated"]:
    ...
    with col2:
        st.title("🔒 StockWise 로그인")
        st.write("앱에 접속하려면 비밀번호를 입력하세요.")
        ...
        if st.button("접속하기", type="primary", use_container_width=True):
            if password == "0420":
                ...
                st.rerun() # 화면 새로고침하여 앱 진입
            else:
                st.error("비밀번호가 올바르지 않습니다.")
    
    st.stop() # 인증되지 않으면 아래 코드를 실행하지 않고 멈춤

# --- 이하 인증된 사용자에게만 보여지는 앱 내용 ---
...
if menu == "1. 총 자산 확인":
    ...
    if typ in ["매수", "매도", "양도세매매"]:
        ...
        if not current_favs.empty:
            ...
            st.selectbox("⚡ 빠른 입력 (즐겨찾기 선택)", fav_options, key="fav_selector", on_change=update_form_from_fav)
    ...
    with st.form("tx_form"):
        ...
        if typ in ["매수","매도","양도세매매"]:
            ...
            tick = c2.text_input("티커", key="form_ticker").upper()
            sect = st.selectbox("섹터", SECTOR_OPTIONS, key="form_sector")
            c3,c4,c5 = st.columns(3)
            amt = c3.number_input("단가($)")
            qty = c4.number_input("수량", format="%.4f")
            rate = c5.number_input("환율", value=float(current_rate))
            krw = 0
            if typ=="양도세매매": st.caption("양도세용 (평단가 유지)")
            else: st.caption(f"예상: {amt*qty*rate:,.0f}원")
        else:
            tick="CASH"; sect="-"; amt=0; qty=1; rate=1.0
            krw = st.number_input("금액(원)")
            st.info("입출금/배당/수수료는 원화 기준")
    ...
    if df.empty: st.warning("내역 없음")
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            title = {'text': f"공제 사용: {deduction_used:,.0f}원"},
        ...
        if gains:
            st.dataframe(pd.DataFrame(gains).style.format({"수량":"{:.4f}", "매도금액(KRW)":"{:,.0f}", "매수금액(KRW, FIFO)":"{:,.0f}", "실현손익(KRW)":"{:,.0f}"}).map(color_negative_red, subset=["실현손익(KRW)"]), use_container_width=True)
